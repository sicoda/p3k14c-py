"""
01_Data_Scrubbing_and_Prep.py : Cleans the p3k14c dataset

Input  : p3k14c_dataset.csv, Labs.csv, DatasetFamilyTree.csv (optional)
Output : cleaned_p3k14c.csv, graveyard.csv, unknown_codes.csv

DEPENDENCIES : pandas numpy tqdm ftfy (optional, improves encoding repair)
PYTHON       : 3.10+
"""
...
def main() -> None:
    default_in  = os.path.join(_SCRIPT_DIR, 'p3k14c_dataset.csv')
    default_out = os.path.join(_SCRIPT_DIR, 'cleaned_p3k14c.csv')

    if len(sys.argv) == 1:
        in_path        = default_in
        out_path       = default_out
        graveyard_path = os.path.join(_SCRIPT_DIR, 'graveyard.csv')
        print('[scrub] No arguments given — using defaults.')
        print(f'[scrub] Input    : {in_path}')
        print(f'[scrub] Output   : {out_path}')
    elif len(sys.argv) in (3, 4):
        in_path        = sys.argv[1]
        out_path       = sys.argv[2]
        graveyard_path = sys.argv[3] if len(sys.argv) == 4 else os.path.join(_SCRIPT_DIR, 'graveyard.csv')
    else:
        print('Usage:')
        print('  python 01_Data_Scrubbing_and_Prep.py                               # defaults')
        print('  python 01_Data_Scrubbing_and_Prep.py <input.csv> <output.csv>')
        print('  python 01_Data_Scrubbing_and_Prep.py <input.csv> <output.csv> <graveyard.csv>')
        sys.exit(1)
