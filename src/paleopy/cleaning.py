"""Data cleaning/scrubbing pipeline for the raw p3k14c dataset.

Ported from Scripts/01_Data_Cleaning_and_Prep.py. Core functions are pure
(take DataFrames/paths as explicit parameters, return DataFrames) so they
can be tested and reused without any file-writing or CLI concerns; those
live in paleopy.cli.clean.
"""
...
from paleopy.utils import fix_encoding, is_nan

pd.options.mode.chained_assignment = None

# ---------------------------------------------------------------------------
# Column-name constants (mirror common.py column-name constants from the
# original repo)
# ---------------------------------------------------------------------------
LAB_ID = "LabID"
LAT = "Lat"
...
# Columns to sanitize (strip exotic quotes, commas, exotic whitespace)
SANITIZE_COLS = [
...
def concat_graveyard(graveyard: pd.DataFrame, new_rows: pd.DataFrame) -> pd.DataFrame:
    """pandas 2.x-safe replacement for the old DataFrame.append()."""
    if new_rows.empty:
        return graveyard
...
def standardize_lab_id(labid: str) -> str:
    """Normalize a lab ID: keep only A-Z and 0-9 (uppercased), then insert a
...
def delete_bad_labs(
    ...
    """Returns (records, graveyard, unknown_codes_df).

    unknown_codes_df is returned rather than written to disk directly (the
    original wrote a hardcoded 'unknown_codes.csv' relative to the CWD) —
    saving is the caller's responsibility.
    """
    print("\n[scrub] Step 2 - Lab-code validation")

    if not labs_path or not os.path.isfile(labs_path):
        print(f"  WARNING: Labs.csv not found at {labs_path}. "
              "Skipping lab-code validation.")
        records = records.set_index(LAB_ID)
        return records, graveyard, pd.DataFrame(columns=["Code"])
    ...
    unknown_df = pd.DataFrame(unknown, columns=["Code"])
    print(f"  Unknown lab codes : {len(unknown)}")
    ...
    # 2g Strip leading whitespace, then standardize format
    records[LAB_ID] = records[LAB_ID].str.lstrip()
    records[LAB_ID] = records[LAB_ID].apply(standardize_lab_id)

    # Set LabID as the DataFrame index (matches original behaviour)
    records = records.set_index(LAB_ID)
    return records, graveyard, unknown_df
...
def _deg_min_sec_to_dec(coord: str, axis: str) -> float:
    """Convert a degree/minute/second string (the '*' char represents the
    degree symbol) to decimal degrees. Mirrors the original degMinSecToDec().
    """
    coord = coord.replace(",", "")
...
def handle_duplicates(
    ...
    family_tree = None
    if family_tree_path and os.path.isfile(family_tree_path):
        family_tree = pd.read_csv(family_tree_path, index_col="Dataset")
...
# ---------------------------------------------------------------------------
# Orchestration — pure, no file I/O beyond reading the optional lookup files
# ---------------------------------------------------------------------------
...
def clean(
    ...
    family_tree_path: str | None = None,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Run the full cleaning pipeline on a raw dataframe.

    Returns (cleaned_df, graveyard_df, unknown_codes_df). Does not write
    any files — that is the CLI layer's responsibility.
    """
    graveyard = pd.DataFrame()

    # Pipeline order matches the official package.
    records, graveyard, unknown_codes = delete_bad_labs(original, graveyard, labs_path)
    records = convert_coordinates(records)
    records = strip_whitespace(records)
    records, graveyard = handle_duplicates(records, graveyard, family_tree_path)
    records, graveyard = apply_misc_scrubbing(records, graveyard)
    records = fix_encoding_cols(records)
    records = fix_columns(records)

    return records, graveyard, unknown_codes
